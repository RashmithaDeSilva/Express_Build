...
import { StandardResponse } from "../util/responses/StandardResponse";
import { getUserService } from "../services/ServicesFactory";
import ErrorResponse from "../util/responses/ErrorResponse";

const router = Router();

let userService: Awaited<ReturnType<typeof getUserService>>;
...
router.get('/users', async (req, res) => {
  try {
    ...
    if (error instanceof Error) {
      await ErrorResponse(error, res, "/users", null);
    } else {
      await ErrorResponse(new Error("Unknown error"), res, "/users", null);
    }
  }
});
...
router.put('/users/:id', async (req, res) => {
  try {
    ...
    if (error instanceof Error) {
      await ErrorResponse(error, res, `/users/${req.params.id}`, null);
    } else {
      await ErrorResponse(new Error("Unknown error"), res, `/users/${req.params.id}`, null);
    }
  }
});
